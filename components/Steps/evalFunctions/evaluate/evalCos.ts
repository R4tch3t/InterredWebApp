...
import { DoubleStr, isNumber, ShaveStr, cosStr } from '../mathString/mathString';
import { Context } from '../process/context';
import { Pstrltx } from './evaluate';

export const EvalCos = (auxStr: string, context: Context) => {
    ...
    context.strDevelopment = context.strDevelopment.split("c" + auxStr).join("c(" + auxStr + ")")
    context.str2 = "c(" + auxStr + ")"
    ...
    if (isNumber(auxStr)) {
        ...
        if (context.MoreDVal !== 1 && context.toDecimalVal === 1) {
            context.res = ShaveStr((cosStr(auxStr, context.DegRad)), 16)
            context.res = context.res.split('e+').join("e")
        } else {
            context.res = cosStr(auxStr, context.DegRad)
        }

        context.res = cleanR(context.res)
        context.str2 = context.str2.split('c+').join("c")
