...
import { DoubleStr, isFrac, isNumber } from '../mathString/mathString';
import { minusstr } from '../mathString/minusStr';
import { Pstrltx } from './evaluate';
import { nDigits } from './expresion';
export const EvalMinus = (auxStr: string, aux1Str: string, context: Context) => {
    ...
    if (isNumber(auxStr) && isNumber(aux1Str)) {
        ...
        context.strDevelopment = context.strDevelopment.split(auxStr + aux1Str).join(aux1Str + auxStr)
        context.strDevelopment = context.strDevelopment.split(aux1Str + auxStr).join(auxStr + aux1Str)        
