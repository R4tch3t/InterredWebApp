...
import { DoubleStr, isNumber, log2Str, ShaveStr } from '../mathString/mathString';
import { Context } from '../process/context';
import { Pstrltx } from './evaluate';

export const EvalLog2 = (auxStr: string, context: Context) => {
    ...
    context.str2 = "log_2(" + auxStr + ")"
    auxStr = DoubleStr(auxStr)
    context.strDevelopment = context.strDevelopment.split("log_2" + auxStr).join(context.str2)

    if (isNumber(auxStr)) {
        context.StepsC += 1
        auxStr = auxStr.split("pi").join(Math.PI.toString())
        if (context.MoreDVal !== 1 && context.toDecimalVal === 1) {
            context.res = ShaveStr(log2Str(auxStr), 16)
            context.res = context.res.split("e+").join("e")
        } else {
            context.res = log2Str(auxStr)
        }

        context.res = cleanR(context.res)

        context.str1 = strToLang("Paso") + context.StepsC + ": quad"
        context.strDevelopment = context.strDevelopment.split("log2" + auxStr).join("log_2(" + auxStr + ")")
        ...
        if (context.toDecimalVal === 1) {
            ...
            if (context.str2.includes(".")) {
                context.str2 = scanNumbers("log_2(" + auxStr + ")", false)
                context.str2 = context.str2.split("log_2").join("log_2(") + ")"
